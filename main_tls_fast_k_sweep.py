...
def find_elbow_point(k_values: List[int], nll_values: List[float]) -> int:
    """
    Identifies the 'elbow' k using the geometric method (max distance from the 
    line connecting the first and last points of the NLL curve).
    """
    if len(k_values) < 3:
        return k_values[0]

    # Coordinates of the start and end points of the curve
    p1 = np.array([k_values[0], nll_values[0]])
    p2 = np.array([k_values[-1], nll_values[-1]])
    ...
    for k, nll in zip(k_values, nll_values):
        p0 = np.array([k, nll])
        # Perpendicular distance formula: |(y2-y1)x0 - (x2-x1)y0 + x2y1 - y2x1| / distance(p1,p2)
        # Or vector cross product magnitude divided by base length
...
def plot_k_sweep_summary(results: List[Dict], output_path: str):
    """
    Plots the NLL curve (with Elbow) and the Exponential Score curve (with Min)
    on a dual-axis chart.
    """
    k_vals = [r['k'] for r in results]
    nlls = [r['nll'] for r in results]
    scores = [r['exp_score'] for r in results]
    
    optimal_k = next((r['k'] for r in results if r['is_exp_optimal']), k_vals[0])
    elbow_k = next((r['k'] for r in results if r['is_elbow']), k_vals[0])

    fig, ax1 = plt.subplots(figsize=(10, 6))

    # --- Axis 1: NLL (Elbow) ---
    color = 'tab:blue'
    ax1.set_xlabel('k (Number of Multi-type Potencies)', fontsize=12)
    ax1.set_ylabel('Negative Log-Likelihood (NLL)', color=color, fontsize=12)
    ax1.plot(k_vals, nlls, color=color, marker='o', linestyle='-', label='NLL')
    ax1.tick_params(axis='y', labelcolor=color)
    
    # Mark Elbow
    elbow_nll = next(n for k, n in zip(k_vals, nlls) if k == elbow_k)
    ax1.plot(elbow_k, elbow_nll, 'o', ms=12, mec='black', mfc='none', markeredgewidth=2, label=f'Elbow (k={elbow_k})')

    # --- Axis 2: Exp Score (Optimization) ---
    ax2 = ax1.twinx()  
    color = 'tab:red'
    ax2.set_ylabel('Penalized Score', color=color, fontsize=12) 
    ax2.plot(k_vals, scores, color=color, marker='s', linestyle='--', label='Exp Score')
    ax2.tick_params(axis='y', labelcolor=color)

    # Mark Optimal
    opt_score = next(s for k, s in zip(k_vals, scores) if k == optimal_k)
    ax2.plot(optimal_k, opt_score, 's', ms=12, mec='black', mfc='none', markeredgewidth=2, label=f'Optimal (k={optimal_k})')
    ...
    plt.title("Structure Search: k-Sweep Summary", pad=30, fontsize=14, fontweight='bold')
    plt.grid(True, linestyle=':', alpha=0.6)
    
    plt.tight_layout()
    plt.savefig(output_path, dpi=300)
    plt.close()
    print(f"✅ Saved k-sweep summary plot to {output_path}")
...
def main_tls(
    ...
    start_k: int = 5,    
    end_k: int = 9,
    out_csv: str = "tls_k_sweep.csv",
    log_dir: Optional[str] = "tls_k_sweep_logs",
):
    random.seed(42) 
    unit_drop_edges = False 
    ...
    try:
        ...
        # 2. Precompute B_sets and Total N (Total leaves for BIC-like n)
        all_B_sets = [compute_B_sets(tree, ltm) for tree, ltm in zip(trees, leaf_type_maps)]
        # Calculate Exact N using recursive count
        total_nodes_n = sum(count_nodes(t) for t in trees)
        print(f"Total nodes (N) across all trees: {total_nodes_n}")

        # 3. Precompute Candidate Pool
        pool = collect_fitch_multis(S, trees, leaf_type_maps)
        fitch_probs_list = compute_fitch_potency_probs(S, trees, leaf_type_maps)
        fitch_probs_dict = {p: prob for p, prob in fitch_probs_list}
        ...
        # --- MAIN LOOP: K-Sweep ---
        print(f"\n{'='*60}")
        print(f"STARTING K-SWEEP (k={start_k} to {end_k})")
        print(f"{'='*60}")

        for k in range(start_k, end_k + 1):
            print(f"\n>>> Processing k={k}...")
            
            priors = Priors(potency_mode="fixed_k", fixed_k=k, rho=0.2)

            # A. Phase 1: MCMC-Z
            bestF_Z, _, _ = run_mcmc_only_Z_parallel(
                S=S, trees=trees, leaf_type_maps=leaf_type_maps,
                priors=priors, unit_drop_edges=unit_drop_edges,
                fixed_k=k, steps=iters, burn_in=(iters * 15) // 100,
                thin=10, base_seed=123, candidate_pool=pool,
                block_swap_sizes=(1,), n_chains=restarts, num_cores=num_cores,
                fitch_probs=fitch_probs_dict
            )

            if bestF_Z is None:
                print(f"[WARN] Failed to find structure for k={k}. Skipping.")
                continue

            # B. Phase 2 & 3 & 4: Flow and Map Construction
            Z_map = bestF_Z.Z_active
            A_full = _full_edges_for_Z(Z_map, unit_drop_edges)
            F_full = Structure(S, Z_map, A_full, unit_drop=unit_drop_edges)
            
            viterbi_flow = calculate_viterbi_flow(trees, F_full, all_B_sets, leaf_type_maps, num_cores=num_cores)
            
            S_nodes = {frozenset([t]) for t in S}
            A_map_final = build_A_map_from_flow(viterbi_flow, Z_map, S_nodes, z_score_threshold=1.5)
            
            bestF_final = Structure(S, Z_map, A_map_final, unit_drop_edges)

            # C. Score
            # Get the raw Log Likelihood (sum of log P(T|F)) without priors
            raw_log_likelihood = get_log_likelihood(bestF_final, trees, leaf_type_maps, all_B_sets)
            
            # --- CALCULATE METRICS ---
            nll = -raw_log_likelihood
            # Use log(total_nodes_n)
            penalty_term = math.log(total_nodes_n) * LAMBDA_VAL * math.exp(ALPHA_VAL * k)
            exp_score = nll + penalty_term
            
            num_edges = len(edges_from_A(bestF_final.A))

            print(f"   [Result k={k}] NLL: {nll:.4f} | Penalty: {penalty_term:.4f} | Exp Score: {exp_score:.4f}")
            ...
            results.append({
                ...
                "exp_score": exp_score,
                "num_edges": num_edges,
                "struct": bestF_final # Save structure if we want to plot the winner later
            })

            # NOTE: We intentionally removed intermediate plotting (traces, structures) here.
        ...
        # 1. Identify Optimal (Min Exp Score)
        best_run = min(results, key=lambda x: x['exp_score'])
        optimal_k = best_run['k']

        # 2. Identify Elbow (on NLL curve)
        k_vals = [r['k'] for r in results]
        nll_vals = [r['nll'] for r in results]
        elbow_k = find_elbow_point(k_vals, nll_vals)

        # 3. Mark booleans
        for r in results:
            r['is_exp_optimal'] = (r['k'] == optimal_k)
            r['is_elbow'] = (r['k'] == elbow_k)

        # 4. Save CSV
        with open(out_csv, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["k", "NLL", "Penalty", "Exp_Score", "Num_Edges", "Is_Optimal", "Is_Elbow"])
            for r in results:
                # Calculate penalty again just for CSV clarity
                pen = r['exp_score'] - r['nll']
                writer.writerow([
                    r['k'], f"{r['nll']:.4f}", f"{pen:.4f}", f"{r['exp_score']:.4f}", 
                    r['num_edges'], r['is_exp_optimal'], r['is_elbow']
                ])
        print(f"\nResults saved to {out_csv}")

        # 5. Generate the Summary Plot
        summary_plot_path = os.path.join(log_dir, "k_sweep_summary.png")
        plot_k_sweep_summary(results, summary_plot_path)

        print(f"\n{'-'*60}")
        print(f"SWEEP COMPLETE.")
        print(f"Optimal k (Exp Score): {optimal_k}")
        print(f"Elbow k (NLL):         {elbow_k}")
        print(f"{'-'*60}")
...
if __name__ == "__main__":
    main_tls(
        ...
        start_k=5,
        end_k=9,
        out_csv="tls_k_sweep.csv",
        log_dir="tls_fast_k_sweep_run_logs"
    )